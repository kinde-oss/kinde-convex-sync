...
export class KindeSync {
  ...
  constructor(
    ...
    // rebuilt (and refetched) on every webhook. The JWKS URL is what binds a
    // webhook to this tenant/environment — see the verification note below.
    ...
    this.webhookHandler = httpActionGeneric(async (ctx, request) => {
      ...
      try {
        ...
        // the JWKS URL we fetch keys from (that URL is derived from this
        // deployment's KINDE_ISSUER_URL), so signature verification alone is the
        // correct binding here. `iss` checks belong on access tokens elsewhere,
        // not on webhooks.
      ...
      // Email is optional on every event type: phone-only Kinde users arrive
      // with email "" or null, which are perfectly valid. Normalize both the
      // empty string and null/undefined to `undefined` so we store an absent
      // email rather than a blank value that could collide on by_email. Phone is
      // treated the same way (Kinde sends it at data.user.phone).
      const emailRaw = user['email'];
      const email =
        typeof emailRaw === 'string' && emailRaw ? emailRaw : undefined;
      const phoneRaw = user['phone'];
      const phone =
        typeof phoneRaw === 'string' && phoneRaw ? phoneRaw : undefined;
